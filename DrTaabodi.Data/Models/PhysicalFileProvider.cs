...
public class PhysicalFileProvider : PhysicalFileProviderBase
{
    ...
    private AccessDetails AccessDetails = new();
    ...
    public void RootFolder(string name)
    {
        ...
        if (!string.IsNullOrEmpty(hostName))
            hostPath = Path.DirectorySeparatorChar + hostName + Path.DirectorySeparatorChar +
                       contentRootPath.Substring(contentRootPath.ToLower().IndexOf(hostName) + hostName.Length + 1);
    }
    ...
    public virtual FileManagerResponse GetFiles(string path, bool showHiddenItems,
        params FileManagerDirectoryContent[] data)
    {
        ...
        try
        {
            ...
            var parentPath = string.IsNullOrEmpty(hostPath)
                ...
                : new DirectoryInfo(hostPath + (path != "/" ? path : "")).Parent.FullName;
            ...
            if (!hasAccess(directory.FullName) || cwd.Permission != null && !cwd.Permission.Read)
            {
                ...
                throw new UnauthorizedAccessException("'" + cwd.Name +
    ...
    protected virtual IEnumerable<FileManagerDirectoryContent> ReadFiles(DirectoryInfo directory, string[] extensions,
        bool showHiddenItems, params FileManagerDirectoryContent[] data)
    {
        try
        {
    ...
    protected string GetRelativePath(string rootPath, string fullPath)
    {
        if (!string.IsNullOrEmpty(rootPath) && !string.IsNullOrEmpty(fullPath))
        {
            ...
            if (!string.IsNullOrEmpty(hostName))
            {
                if (rootPath.Contains(hostName) || rootPath.ToLower().Contains(hostName) ||
                    rootPath.ToUpper().Contains(hostName))
                    rootPath = rootPath.Substring(
                        rootPath.IndexOf(hostName, StringComparison.CurrentCultureIgnoreCase) + hostName.Length);
                if (fullPath.Contains(hostName) || fullPath.ToLower().Contains(hostName) ||
                    fullPath.ToUpper().Contains(hostName))
                    fullPath = fullPath.Substring(
                        fullPath.IndexOf(hostName, StringComparison.CurrentCultureIgnoreCase) + hostName.Length);
            ...
            {
                if (fullPath.Contains(rootDirectory.FullName)) return fullPath.Substring(rootPath.Length - 1);
            }
    ...
    protected virtual IEnumerable<FileManagerDirectoryContent> ReadDirectories(DirectoryInfo directory,
        string[] extensions, bool showHiddenItems, params FileManagerDirectoryContent[] data)
    {
    ...
    public virtual FileManagerResponse Create(string path, string name, params FileManagerDirectoryContent[] data)
    {
        ...
        try
        {
            ...
            if (PathPermission != null && (!PathPermission.Read || !PathPermission.WriteContents))
            {
                ...
                throw new UnauthorizedAccessException("'" + getFileNameFromPath(rootName + path) +
            ...
            if (directoryExist)
            {
                ...
                var er = new ErrorDetails();
                er.Code = "400";
                er.Message = "A file or folder with the name " + exist.Name + " already exists.";
    ...
    public virtual FileManagerResponse Details(string path, string[] names, params FileManagerDirectoryContent[] data)
    {
        ...
        try
        {
            if (names.Length == 0 || names.Length == 1)
            {
                ...
                fileDetails.Size = (File.GetAttributes(fullPath) & FileAttributes.Directory) != FileAttributes.Directory
                ...
                fileDetails.Location =
                    GetRelativePath(
                        string.IsNullOrEmpty(hostName)
                            ...
                            : baseDirectory.Parent.FullName + Path.DirectorySeparatorChar, info.FullName).Substring(1);
            ...
            else
            {
                ...
                var baseDirectoryParentPath = string.IsNullOrEmpty(hostName)
                    ? parentPath
        ...
        try
        {
            ...
            for (var i = 0; i < names.Length; i++)
            {
                ...
                if (permission != null && (!permission.Read || !permission.Write))
                {
                    ...
                    throw new UnauthorizedAccessException("'" + getFileNameFromPath(rootName + path + names[i]) +
            ...
            if (result != string.Empty)
            {
                ...
                var er = new ErrorDetails();
                er.Message = "'" + getFileNameFromPath(deniedPath) +
                             "' is not accessible.  you need permission to perform the write action.";
                er.Code = "401";
                if (er.Code == "401" && !string.IsNullOrEmpty(accessMessage)) er.Message = accessMessage;
    ...
    public virtual FileManagerResponse Rename(string path, string name, string newName, bool replace = false,
        params FileManagerDirectoryContent[] data)
    {
        ...
        try
        {
            ...
            if (isFile)
            {
                if (File.Exists(newPath) && !oldPath.Equals(newPath, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    var er = new ErrorDetails();
                    er.Code = "400";
                    er.Message = "Cannot rename " + exist.Name + " to " + newName + ": destination already exists.";
    ...
    public virtual FileManagerResponse Copy(string path, string targetPath, string[] names, string[] renameFiles,
        FileManagerDirectoryContent targetData, params FileManagerDirectoryContent[] data)
    {
        ...
        try
        {
            ...
            {
                accessMessage = PathPermission.Message;
                throw new UnauthorizedAccessException("'" + getFileNameFromPath(rootName + targetPath) +
                                                      "' is not accessible. You need permission to perform the writeContents action.");
            }
            ...
            for (var k = 1; k < missingFiles.Count; k++) namelist = namelist + ", " + missingFiles[k];
    ...
    public virtual FileManagerResponse Move(string path, string targetPath, string[] names, string[] renameFiles,
        FileManagerDirectoryContent targetData, params FileManagerDirectoryContent[] data)
    {
        ...
        try
        {
            ...
            if (result != string.Empty)
            {
                ...
                var er = new ErrorDetails();
                er.Message = "'" + getFileNameFromPath(deniedPath) +
                             "' is not accessible. You need permission to perform this action.";
                er.Code = "401";
    ...
    public virtual FileManagerResponse Search(string path, string searchString, bool showHiddenItems = false,
        bool caseSensitive = false, params FileManagerDirectoryContent[] data)
    {
    ...
    public virtual FileStreamResult GetImage(string path, string id, bool allowCompress, ImageSize size,
        params FileManagerDirectoryContent[] data)
    {
        try
        {
                ...
                if (uploadFiles != null)
                {
                    ...
                    else if (action == "keepboth")
                    {
                        ...
                        while (File.Exists(newName + (fileCount > 0
                                   ...
                                   : Path.GetExtension(name)))) fileCount++;
        ...
        protected virtual void AddFileToZip(string zipFileName, string fileToAdd)
        {
            using (Package zip = System.IO.Packaging.Package.Open(zipFileName, FileMode.OpenOrCreate))
            {
                ...
                if (zip.PartExists(uri))
                    zip.DeletePart(uri);
                PackagePart pkgPart =
 zip.CreatePart(uri, System.Net.Mime.MediaTypeNames.Application.Zip, CompressionOption.Normal);
    ...
    public virtual FileStreamResult Download(string path, string[] names, params FileManagerDirectoryContent[] data)
    {
        try
        {
            ...
            for (var i = 0; i < names.Length; i++)
            {
                ...
                if (FilePermission != null && (!FilePermission.Read || !FilePermission.Download))
                    throw new UnauthorizedAccessException("'" + rootName + path + names[i] +
                                                          "' is not accessible. Access is denied.");

                fullPath = Path.Combine(contentRootPath + path, names[i]);
                if ((File.GetAttributes(fullPath) & FileAttributes.Directory) != FileAttributes.Directory) count++;
            }
    ...
    protected virtual FileStreamResult DownloadFile(string path, string[] names = null)
    {
        try
        {
            ...
            {
                var fileName = Guid.NewGuid() + "temp.zip";
                        ...
                        try
                        {
                            using (archive = ZipFile.Open(tempPath, ZipArchiveMode.Update))
                            {
                                currentDirectory = Path.Combine(contentRootPath + path, names[i]);
                                    ...
                                    zipEntry =
 archive.CreateEntryFromFile(Path.Combine(this.contentRootPath, currentDirectory), names[i]);
#else
                                zipEntry = archive.CreateEntryFromFile(Path.Combine(contentRootPath, currentDirectory),
                                    names[i], CompressionLevel.Fastest);
#endif
                            }
                        }
                        catch (Exception)
...
#if SyncfusionFramework4_5
                                    zipEntry =
 archive.CreateEntryFromFile(filePath, names[i] + filePath.Substring(currentDirectory.Length));
#else
                                    zipEntry = archive.CreateEntryFromFile(filePath,
                                        names[i] + filePath.Substring(currentDirectory.Length),
                                        CompressionLevel.Fastest);
#endif
                                }

                            foreach (var filePath in Directory.GetDirectories(currentDirectory, "*",
                                         SearchOption.AllDirectories))
                                if (Directory.GetFiles(filePath).Length == 0)
                                {
#if SyncfusionFramework4_5
                                            zipEntry =
 archive.CreateEntryFromFile(Path.Combine(this.contentRootPath, filePath), filePath.Substring(path.Length));
#else
                                    zipEntry = archive.CreateEntry(names[i] +
                                                                   filePath.Substring(currentDirectory.Length) + "/");
#endif
                                }
                        }
                        else
 ...
 archive.CreateEntryFromFile(Path.Combine(this.contentRootPath, currentDirectory), names[i]);
    ...
    private string DirectoryRename(string newPath)
    {
        ...
        while (Directory.Exists(newPath + (directoryCount > 0 ? "(" + directoryCount + ")" : ""))) directoryCount++;
        newPath = newPath + (directoryCount > 0 ? "(" + directoryCount + ")" : "");
        return newPath;
    }
    ...
    private string FileRename(string newPath, string fileName)
    {
        ...
        while (File.Exists(newPath + (fileCount > 0
                   ...
                   : Path.GetExtension(fileName)))) fileCount++;
    ...
    protected virtual AccessPermission GetPermission(string location, string name, bool isFile)
    {
        ...
        if (isFile)
        {
            ...
            foreach (var fileRule in AccessDetails.AccessRules)
                if (!string.IsNullOrEmpty(fileRule.Path) && fileRule.IsFile &&
                    (fileRule.Role == null || fileRule.Role == AccessDetails.Role))
                {
        ...
        foreach (var folderRule in AccessDetails.AccessRules)
            if (folderRule.Path != null && folderRule.IsFile == false &&
                (folderRule.Role == null || folderRule.Role == AccessDetails.Role))
            {
                ...
                else if (GetPath(folderRule.Path) == GetValidPath(location + name) || GetPath(folderRule.Path) ==
                         GetValidPath(location + name + Path.DirectorySeparatorChar))
                {
                    FilePermission = UpdateFolderRules(FilePermission, folderRule);
                }
    ...
    protected virtual string[] GetFolderDetails(string path)
    {
        ...
        var parentPath = "";
        for (var i = 0; i < str_array.Length - 2; i++) parentPath += str_array[i] + "/";
    ...
    private long GetDirectorySize(DirectoryInfo dir, long size)
    {
        try
        {
            foreach (var subdir in dir.GetDirectories()) size = GetDirectorySize(subdir, size);
    ...
    private List<FileInfo> GetDirectoryFiles(DirectoryInfo dir, List<FileInfo> files)
    {
        try
        {
            foreach (var subdir in dir.GetDirectories()) files = GetDirectoryFiles(subdir, files);
            foreach (var file in dir.GetFiles()) files.Add(file);
        }
        catch (Exception e)
        {
            if (e.GetType().Name != "UnauthorizedAccessException") throw e;
    ...
    private List<DirectoryInfo> GetDirectoryFolders(DirectoryInfo dir, List<DirectoryInfo> files)
    {
        try
        {
            foreach (var subdir in dir.GetDirectories()) files = GetDirectoryFolders(subdir, files);
            foreach (var file in dir.GetDirectories()) files.Add(file);
        }
        catch (Exception e)
        {
            if (e.GetType().Name != "UnauthorizedAccessException") throw e;
